...
- 电脑端控制台展示 DG-LAB 4 App V4 绑定二维码、手机游戏二维码、手机 HTTPS 证书入口、局域网地址、端口和延迟状态。
- V4 桥接支持郊狼 2.0 与 3.0，按硬件型号自动发送 V2 三字节或 V3 八字节波形。
- 电脑端和手机选择页展示设备型号、蓝牙连接、强度、限幅、电量、静音和过热状态；多设备必须明确选择控制目标。
- 电脑端使用三个固定页签管理状态、证书和技术信息，置顶栏始终保留 A/B 紧急停止；试电由后端强制限制强度、时长和冷却。
- 手机端包含手抖挑战、保持角度、摇骰子对决、极速角子机四个小游戏；设置按基础与高级分组折叠，底部固定保留恢复默认、校准和开始操作。
- 手机浏览器各页面始终显示设备状态和 A/B 紧急停止；Android APK 使用原生停止按钮并在连接期间保持屏幕常亮。
- 手抖挑战、保持角度、摇骰子对决和极速角子机均支持按玩法配置输出节奏，避免连续输出叠得过密。
- 四个游戏共用一个“输出感觉”选择器，提供短时立即有感的游戏默认、按时长随机和 16 种经典波形。
- 后端内置精简 V4 桥接层，统一执行 A/B 限幅、时长限制、请求限流、自动到期强度和停止输出。
...
设备控制要求 DG-LAB 4 App。扫描控制台的 V4 二维码后，在 App 中添加并连接郊狼 2.0 或 3.0；控制台显示设备已连接且所选通道上限有效后才能输出。
...
- `tests/test_ui_structure.py`：页签、折叠设置、紧急停止和 Android 工具栏结构测试。
...
App 尚未回传通道限幅、限幅为 0、通道静音或过热时，后端拒绝对应输出；A+B 模式要求两路都安全。所有硬件输出最多只运行 1 个任务，重叠请求不会进入等待队列。

连接测试只用于确认链路和通道，不替代人工确认。安全试电默认强度为 15，最高 30，最长 1 秒，固定使用游戏默认波形并带有后端冷却。

手机端置顶停止会取消当前游戏的全部待执行队列、清空 A/B 并退回玩法列表；离开网页、进入后台、刷新或 WebSocket 断开时，服务端也会兜底清空两路输出。

V4 非零强度全部使用自动到期任务；即使电脑异常退出，App 端也会在本次输出期限加 0.8 秒余量后自行归零。紧急停止仍会额外清理任务并逐路发送强度 0。
...
手机选择页展示紧急停止方式和 A/B 通道说明。游戏页可选择输出感觉和 A/B 通道，但不能修改波形底层参数、软上限或平衡参数。
