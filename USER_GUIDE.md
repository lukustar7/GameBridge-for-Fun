# GameBridge for Fun 1.1.0-beta.3 用户手册
...
1. 安装项目 `APK/GameBridgeForFun-Android15-v1.1.0-beta.3.apk` 并打开；该包使用项目独立正式签名，不需要安装网页 HTTPS 根证书。
...
第一版还会显示“当前玩法强度硬上限”。该值由设备 App 回传的 A/B 限幅、所选通道和 B 比例共同计算；游戏滑杆、旧设置恢复、发送前检查和电脑后台都会再次执行这个上限。App 未连接或限幅未读取时不会把未知值当作 200，也不会允许正式输出。
