...
骰子“每下”、角子机“满槽惩罚”和“没中奖轻电”都按一次完整惩罚计算，用户设置与后端实际执行均不得短于 `1 秒`；旧版保存的 `0.x 秒` 设置会自动提升到 `1 秒`。手抖挑战、保持角度使用可以随时续发和中断的内部短控制帧，不把内部技术间隔暴露成游戏参数。
...
适合使用 HTTPS 游戏入口。开始前可以点击参数上方的 `校准姿态`。
