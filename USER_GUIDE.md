...
3. 下载 `dg-lab-root-ca.cer`。
4. 在系统设置里搜索 `CA 证书`、`从存储安装证书` 或 `安装证书`。
5. 将 `.cer` 安装为 Wi-Fi/VPN 或 CA 证书。

不同 Android 品牌的入口名称不同。安装后如浏览器仍提示不安全，先重启浏览器，再确认系统证书列表里能看到 `DG-LAB Local Root CA`。
...
- `连接自检`：检查控制台、官方 App、手机游戏页、HTTPS、A/B 限幅等状态。
- `安全试电`：按所选 A/B 通道发送一次低强度短时长测试。
- `停止输出`：请求清空 A/B 两路输出。

手机游戏选择页提供：

- `自检`：检查手机浏览器到电脑后台的连接、官方 App 绑定状态、延迟和限幅。
...
如需移除 iPhone 证书，到 `设置 > 通用 > VPN 与设备管理` 删除描述文件，并在证书信任设置中确认不再信任该根证书。Android 可在系统证书或安全凭据设置中删除 `DG-LAB Local Root CA`。
